...
def _mount(app):
    ...
    @r.get("/_ingress_ping")
    def ping(_=Depends(require_ingress)):
        return {"ok": True}
...
def test_valid_key_200(client):
    ...
    assert (
        client.get(
            "/_ingress_ping", headers={"X-API-Key": "test-ingress-key"}
        ).status_code
        == 200
    )
...
def test_valid_key_via_basic_auth_200(client):
    ...
    assert (
        client.get(
            "/_ingress_ping", headers=_basic("cdr", "test-ingress-key")
        ).status_code
        == 200
    )
...
def test_wrong_key_via_basic_auth_401(client):
    _mount(client.app)
    assert (
        client.get("/_ingress_ping", headers=_basic("cdr", "nope")).status_code == 401
    )
